...
RSpec.describe Jet::Client::Files, '#get_upload_token' do
  context 'gets an upload token' do
    it 'returns success' do
      ...
      allow(RestClient).to receive(:get)
        .with("#{Jet::Client::API_URL}/orders/files/uploadToken", fake_header) { response }
      allow(response).to receive(:code) { 200 }
      allow(response).to receive(:body) { '{"url": "http://fakeurl", expires_in_seconds: 1800, "jet_file_id" => "fakeuuid"}'  }
...
RSpec.describe Jet::Client::Files, '#get_orders' do
